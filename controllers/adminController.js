...
async function displayAllAdmin() {
  try {
    const admins = await Admin.find({}, '-password'); //display all information except for password
...
module.exports = displayAllAdmin;
...
async function updatePassword(req, res) {
  try{
    ...
    //Validate current password by comparing the input with password recorded
    const isPasswordValid = await bcrypt.compare(currentPassword, admin.password);
    if(!isPasswordValid) {
      return res.status(400).json({
        message: 'The password you entered does not match current password.'
      });
    }
